# promoter-catalog-search

## Purpose

Búsqueda por texto en el catálogo administrable de Promotores, alineada con el patrón ya existente en Unidades Orgánicas.

## Requirements

### Requirement: Búsqueda por nombre en el catálogo de Promotores
El sistema SHALL permitir filtrar el listado de promotores (`GET /api/promoters`) por un parámetro de texto opcional `q`, que coincide con promotores cuyo `Name` contiene el texto buscado (coincidencia parcial), de forma insensible a mayúsculas/minúsculas y a acentos/diacríticos.

#### Scenario: Búsqueda con coincidencias
- **WHEN** un cliente autenticado llama a `GET /api/promoters?q=uni`
- **THEN** el sistema devuelve un `PagedResult<PromoterDto>` que incluye únicamente los promotores cuyo `Name` contiene "uni"

#### Scenario: Búsqueda insensible a mayúsculas y acentos
- **WHEN** un cliente llama a `GET /api/promoters?q=promocion` y existe un promotor con `Name` = "Vicerrectorado de Promoción"
- **THEN** el sistema incluye ese promotor en el resultado, sin importar que la búsqueda no incluya tilde ni coincida en mayúsculas/minúsculas
