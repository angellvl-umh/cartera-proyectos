...
- **Agente IA (chat nativo):** panel de chat integrado en la app (`/api/chat/*`: crear/listar conversaciones, mensajes, envío con bucle de tool-calling); catálogo de ~45 tools en `ChatToolCatalog` (`Core/Features/Chat/Tools`, particionado por área: `.Tasks`, `.Projects`, `.Persons`, `.Sprints`, `.Epics`, `.Teams`, `.Backlog`, `.Portfolio`, `.Catalogs`, `.Exports`, `.Charts`) — mis tareas, proyectos, detalle proyecto, capacidad, búsqueda semántica, cambiar estado de tarea, crear tarea, comentarios, notas, avance semanal, informe de cartera, reindexar embeddings, CRUD de personas (solo Gestor), CRUD de proyectos y cambio de estado, riesgos y dependencias, asignación de equipo a proyecto, **sprints** (listar, burndown, crear, activar/completar con carry-over), **épicas** (listar, crear, editar), **equipos** (listar, actividad), **backlog** (reordenar, asignación masiva a sprint), **roadmap de cartera y forecast de capacidad**, **métricas ágiles** (velocity, cycle/lead time), **catálogos** (promotores, unidades orgánicas, tags), **exportación a Excel** (`export_projects_excel`, `export_weekly_portfolio_report_excel` vía `ClosedXML`) y **gráficos** (`chart_team_capacity`, `chart_project_progress`, `chart_my_tasks_by_status`, `chart_projects_by_status`, `chart_projects_by_team` — SVG construido a mano en el backend, sin librería de gráficos, mismo criterio que los charts SVG del frontend); tools de lectura reutilizan directamente los `IRequest` ya usados por sus endpoints REST equivalentes, tools de escritura se envuelven en un `Agent*Command` fino (`Core/Features/Agent/*`) que implementa `IAgentAuditable` para quedar auditadas; export/charts se sirven como ficheros/imágenes efímeras vía `IEphemeralBlobStore` (`IMemoryCache`, expiración deslizante de 20 min) detrás de `GET /api/chat/{charts,exports}/{id:guid}` (capability-URL sin autenticación, id no adivinable) y `IPublicUrlProvider` (`Chat:PublicBaseUrl`, no HttpContext — las tools corren fuera del pipeline HTTP); identidad resuelta del JWT de la petición (`CurrentUser.ResolveAsync`), nunca de un parámetro del modelo; modelo configurado por la app (`Chat:Model`), no elegible por el usuario; conversaciones/mensajes persistidos (`Conversation`/`ChatMessage`) por usuario; auditoría vía `AgentActionLog`/`AgentAuditBehavior`
...
- **Arquitectura del catálogo de tools del agente:** la lógica compartida entre cada handler de dominio (REST) y su equivalente `Agent*Handler` vive en servicios de aplicación planos en `Features/<Feature>/` (`IWorkItemLifecycleService`, `IProjectLifecycleService`, `IProjectGovernanceService`, `ISprintLifecycleService`, `IEpicService`, `IPersonManagementService`, servicios de lectura en `Features/Agent/AgentReadServices.cs` + `IWeeklyPortfolioReportService`) — ningún `IRequestHandler` inyecta `ISender`/`IMediator` salvo `SendChatMessageHandler` (orquestador del tool-calling), regla verificada por `tests/CarteraProyectos.ArchTests`; convención detallada en `.ai/skills/dotnet10/SKILL.md`
- 500+ tests unitarios + `tests/CarteraProyectos.ArchTests` (regla de arquitectura sobre MediatR)
